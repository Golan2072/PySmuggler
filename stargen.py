...
class Star:
    def atmo_gen(self):
        if self.size == 0:
            ...
            atmosphere = stellagama.dice(2, 6) - 7 + self.size
        if atmosphere > 15:
            atmosphere = 15
        if atmosphere < 0:
            atmosphere = 0
    ...
    def hydro_gen(self):
        hydrographics = stellagama.dice(2, 6) - 7 + self.size
        if self.size <= 1:
            hydrographics = 0
    ...
    def gov_gen(self):
        government = stellagama.dice(2, 6) - 7 + self.population
    ...
    def law_gen(self):
        law = stellagama.dice(2, 6) - 7 + self.government
    ...
    def starport_gen(self):
        starport_roll = stellagama.dice(2, 6) - 7 + self.population
        starport = " X "
        if starport_roll <= 2:
            starport = " X "
    ...
    def __init__(self, hex_column, hex_row):
        if stellagama.dice(1, 6) >= 4:
            self.size = stellagama.dice(2, 6) - 2
